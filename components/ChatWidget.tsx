...
const ChatWidget: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      <div
        className={`fixed bottom-24 right-6 w-80 md:w-96 bg-slate-900 rounded-2xl shadow-2xl z-50 flex flex-col border border-slate-800 transition-all duration-300 origin-bottom-right transform ${
          isOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0 pointer-events-none'
        }`}
        style={{ height: '500px', maxHeight: '80vh' }}
      >
        {/* Header */}
        <div className="bg-slate-950 p-4 rounded-t-2xl flex items-center gap-3 border-b border-slate-800">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-950/60">
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="bg-slate-800 p-3 rounded-2xl rounded-bl-none shadow-sm border border-slate-700 flex gap-1 items-center">
        ...
        <div className="p-3 bg-slate-900 border-t border-slate-800 rounded-b-2xl">
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 bg-slate-800 border-0 rounded-full px-4 py-2 text-sm text-white placeholder:text-slate-500 focus:ring-2 focus:ring-violet-500 outline-none"
            />
            <button
              onClick={handleSend}
